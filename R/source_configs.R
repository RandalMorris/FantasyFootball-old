html_sites <- list(
  ...
  CBS = list(
    ...
    extract_pid = function(p_node){
      ...
      p_node %>% html_nodes(xpath="//*[@id='TableBase']/div/div/table/tbody/tr[1]/td[1]/span[2]/a") %>% html_attr("href") %>% str_extract("[0-9]{2,8}")},
    split_cols = list(
      list(
  ...
  Yahoo = list(
    ...
    stat_cols = c(
      ...
      dst_ret_td = "Return TD", idp_ret_yds = "Return Yds", idp_ret_td = "Return TD",
...
json_sites <- list(
  ...
  FantasyFootballNerd = list(
    ...
    get_path = function(season, week, position){
      ...
      sprintf("%s-projections/json/%s/%s/%s", data_type, "2tn5un7ws52w", position, week_no)
    },
    url_positions = function(p)switch(p, "DST" = "DEF", p),
    min_week = 0,
    max_week = 17,
